...
def Inference2(id, resolution, threshold, exceed_lock, block_lock, block_nums, free_nums, main_pid, mem_size, file_size, base, add_lock, child, wait_queue):
    ...
    print('resnet resolution --', resolution, ':', id, time.time() - start, listener_time, whole_time - listener_time, thread_base)
    print('-------------------------------------------------------------------')
...
if __name__ == '__main__':
    ...
    start = time.time()
    for i in range(pipe_224):
        process.append(mp.Process(target=Inference2, args=(num, 224, threshold << 20, exceed_block, block_lock, block_nums, free_nums, os.getpid(), mem_size[608], file_size, 1, add_lock, child, wait_queue)))
    for i in range(pipe_608 - 1):
